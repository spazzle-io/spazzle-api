...
func handleSubPhaseConfirmArtist(
	...
	_, err := sendGameEvent(ctx, state, notifyCh, gameevents.TypeArtistConfirmed, payload, uuid.Nil, false)
	if err != nil {
...
func handleSubPhaseDrawing(ctx workflow.Context, state *GameState, notifyCh workflow.Channel, logger log.Logger) error {
	...
	_, err := sendGameEvent(
		ctx, state, notifyCh, gameevents.TypeBeginDrawing, beginDrawingPayload, uuid.Nil, false,
	)
	if err != nil {
	...
	_, err = sendGameEvent(
		ctx, state, notifyCh, gameevents.TypeEndDrawing, endDrawingPayload, uuid.Nil, false,
	)
	if err != nil {
...
func sendArtistDisconnectedEvent(ctx workflow.Context, state *GameState, notifyCh workflow.Channel, logger log.Logger) {
	...
	_, err := sendGameEvent(ctx, state, notifyCh, gameevents.TypeArtistDisconnected, payload, uuid.Nil, false)
	if err != nil {
...
func scheduleNextArtistSelection(ctx workflow.Context, state *GameState, notifyCh workflow.Channel, logger log.Logger) {
	...
	delivered, err := sendGameEvent(
		ctx, state, notifyCh, gameevents.TypeNextArtistSelected, payload, nextArtist, true,
	)
	if err != nil {
...
func scheduleHints(ctx workflow.Context, state *GameState, notifyCh workflow.Channel, logger log.Logger) {
	...
	for _, hint := range hints {
		...
		_, err := sendGameEvent(
			ctx, state, notifyCh, gameevents.TypeWordHintRevealed, payload, uuid.Nil, false,
		)
		if err != nil {
